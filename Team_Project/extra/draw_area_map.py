...
for area_val, color in colors.items():
    ax.add_patch(plt.Rectangle((0, 0), 1, 1, color=color, visible=False)) # 범례를 위한 더미 패치
    legend_patches.append(mpatches.Patch(color=color, label=f'Area {area_val}'))

# 각 셀에 색상 채우기
for index, row in df_struct.iterrows():
    x, y, area = row['x'], row['y'], row['area']
    ax.add_patch(plt.Rectangle((x - 0.5, y - 0.5), 1, 1, color=colors.get(area, 'white')))
...
# 이미지 파일 저장
output_path = 'Extra/area_map.png'
