...
structure_coords_df = df_merged[df_merged['category'] > 0][['x', 'y', 'struct']].drop_duplicates(subset=['x', 'y'])
all_structures = [(row['x'], row['y']) for index, row in structure_coords_df.iterrows()]

# Ensure MyHome is the starting point
my_home_coord = (df_merged[df_merged['struct'] == 'MyHome']['x'].iloc[0], df_merged[df_merged['struct'] == 'MyHome']['y'].iloc[0])

# Add MyHome to all_structures if it's not already there (e.g., if its category was 0 initially)
...
while len(visited_structures) < len(all_structures):
    ...
    for structure in structures_to_visit:
        if structure not in visited_structures:
            ...
            if path_segment and (len(path_segment) - 1) < min_path_length: # -1 because path includes start and end
                min_path_length = len(path_segment) - 1
                next_structure = structure
                shortest_segment = path_segment

    if next_structure:
        # Add the path segment (excluding the start node of the segment, as it's current_location)
        optimized_path_coords.extend(shortest_segment[1:])
        current_location = next_structure
        visited_structures.add(next_structure)
    else:
        print(f"Warning: Could not find path from {current_location} to any unvisited structure. Path might be incomplete.")
        break
...
plt.scatter(my_home_coord[0], my_home_coord[1], c='blue', marker='*', s=300, label='Start (MyHome)', zorder=5)
...
structure_legend_elements = [
    ...
    plt.Line2D([0], [0], color='red', lw=2, label='Optimized Path'),
...
plt.gca().add_artist(legend1) # Add the first legend manually so the second one doesn't overwrite it
...
# Save the image
output_image_path = 'Extra/optimized_all_structures_path.png'
...
if optimized_path_coords:
    ...
    output_csv_path = 'outputFiles/optimized_all_structures_path.csv'
