...
# 데이터 병합
df_struct_with_category = pd.merge(df_struct, df_category, on='category', how='left')
df_merged = pd.merge(df_map, df_struct_with_category, on=['x', 'y'], how='left')
...
def bfs(start, end, obstacles, grid_size=15):
    ...
    while queue:
        ...
        for dx, dy in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
            ...
            if 1 <= nx <= grid_size and 1 <= ny <= grid_size and (nx, ny) not in visited and (nx, ny) not in obstacles:
                visited.add((nx, ny))
                new_path = list(path)
...
def plot_map(df_data, path_coords, title, output_image_path, extra_legend_elements=None, start_node=None, visited_structures=None):
    ...
    for index, row in df_data.iterrows():
        ...
        plt.gca().add_patch(plt.Rectangle((x - 0.5, y - 0.5), 1, 1, color=area_colors.get(area, 'white')))
    ...
    for idx, row in construction_sites.iterrows():
        plt.scatter(row['x'], row['y'], c='grey', marker='s', s=200, zorder=4) # Construction sites on top

    # Plot the path
    if path_coords:
        path_x, path_y = zip(*path_coords)
        plt.plot(path_x, path_y, c='red', linewidth=2, zorder=2) # Path below structures but above background

    # Plot start and visited structures for bonus task
    if start_node:
        plt.scatter(start_node[0], start_node[1], c='blue', marker='*', s=300, label='Start (MyHome)', zorder=5)
    if visited_structures:
        for s in visited_structures:
            if s != start_node:
                plt.scatter(s[0], s[1], c='orange', marker='X', s=150, label='Visited Structure', zorder=5)
    ...
    for area_val, color in area_colors.items():
        area_legend_patches.append(mpatches.Patch(color=color, label=f'Area {area_val}'))
    ...
    structure_legend_elements = [
        ...
        plt.Line2D([0], [0], marker='s', color='w', label='Construction Site', markerfacecolor='grey', markersize=10),
        plt.Line2D([0], [0], color='red', lw=2, label='Path')
    ]
    if extra_legend_elements:
        structure_legend_elements.extend(extra_legend_elements)

    legend1 = plt.legend(handles=area_legend_patches, loc='lower left', bbox_to_anchor=(0, 1.02, 0.5, 0.2), mode="expand", borderaxespad=0, title='Area Colors')
    plt.gca().add_artist(legend1)

    legend2 = plt.legend(handles=structure_legend_elements, loc='lower right', bbox_to_anchor=(0.5, 1.02, 0.5, 0.2), mode="expand", borderaxespad=0, title='Map Elements')
...
start_node_cafe = (df_merged[df_merged['struct'] == 'MyHome']['x'].iloc[0], df_merged[df_merged['struct'] == 'MyHome']['y'].iloc[0])
end_node_cafe = (df_merged[df_merged['struct'] == 'BandalgomCoffee']['x'].iloc[0], df_merged[df_merged['struct'] == 'BandalgomCoffee']['y'].iloc[0])
obstacles_cafe = set(zip(df_merged[df_merged['ConstructionSite'] == 1]['x'], df_merged[df_merged['ConstructionSite'] == 1]['y']))
...
if shortest_path_cafe:
    ...
    plot_map(df_merged, shortest_path_cafe, 'Path from MyHome to BandalgomCoffee', 'outputFiles/map_final.png')
else:
...
structure_coords_df = df_merged[(df_merged['category'] > 0) & (df_merged['ConstructionSite'] == 0)][['x', 'y', 'struct']].drop_duplicates(subset=['x', 'y'])
all_structures = [(row['x'], row['y']) for index, row in structure_coords_df.iterrows()]

my_home_coord_all = (df_merged[df_merged['struct'] == 'MyHome']['x'].iloc[0], df_merged[df_merged['struct'] == 'MyHome']['y'].iloc[0])
...
while len(visited_structures_all) < len(all_structures):
    ...
    for structure in structures_to_visit:
        if structure not in visited_structures_all:
            path_segment_all = get_path_segment(current_location_all, structure, obstacles_cafe)
            if path_segment_all and (len(path_segment_all) - 1) < min_path_length_all: 
    ...
    if next_structure_all:
        ...
        print(f"Warning: Could not find path from {current_location_all} to any unvisited structure. Optimized path might be incomplete.")
        break

if optimized_path_coords_all:
    optimized_path_df_all = pd.DataFrame(optimized_path_coords_all, columns=['x', 'y'])
    ...
    extra_legend_elements_all = [
        plt.Line2D([0], [0], marker='*', color='w', label='Path Start (MyHome)', markerfacecolor='blue', markersize=15),
        plt.Line2D([0], [0], marker='X', color='w', label='Visited Structure', markerfacecolor='orange', markersize=10)
    ]
    plot_map(df_merged, optimized_path_coords_all, 'Optimized Path Visiting All Structures', 'outputFiles/optimized_all_structures_path.png', extra_legend_elements=extra_legend_elements_all, start_node=my_home_coord_all, visited_structures=visited_structures_all)
else:
